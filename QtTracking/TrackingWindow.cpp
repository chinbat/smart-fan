...
void TrackingWindow::updateMaskImage(unsigned char * image)
{
    ...
    static QRgb torso = qRgb(0, 0, 0);      // torso joint is black
    qimage.setColor(m_torsoColorIndex, torso);
    static QRgb center = qRgb(255, 100, 255);
    qimage.setColor(m_defaultJointColorIndex, center);

    // mask color
    //static QRgb maskColor = qRgb(200, 64, 64);
    static QRgb maskColor = qRgb(255, 255, 255);    // mask is white
    qimage.setColor(255, maskColor);

    // background color
    static QRgb backgroundColor = qRgb(0, 0, 0);    // background is black
    qimage.setColor(0, backgroundColor);
    if(m_numberOfPoints > 0)
        {
            // display the tracked points
                qimage.setPixel(m_pointsArray[0].point.x()-1,m_pointsArray[0].point.y(),
                                m_pointsArray[0].maskColorIndex);
                qimage.setPixel(m_pointsArray[0].point.x()+1, m_pointsArray[0].point.y(),
                                m_pointsArray[0].maskColorIndex);
                qimage.setPixel(m_pointsArray[0].point.x()-2, m_pointsArray[0].point.y(),
                                m_pointsArray[0].maskColorIndex);
                qimage.setPixel(m_pointsArray[0].point.x()+2, m_pointsArray[0].point.y(),
                                m_pointsArray[0].maskColorIndex);
                qimage.setPixel(m_pointsArray[0].point.x(), m_pointsArray[0].point.y()-1,
                                m_pointsArray[0].maskColorIndex);
                qimage.setPixel(m_pointsArray[0].point.x(), m_pointsArray[0].point.y()+1,
                                m_pointsArray[0].maskColorIndex);
                qimage.setPixel(m_pointsArray[0].point.x(), m_pointsArray[0].point.y()-2,
                                m_pointsArray[0].maskColorIndex);
                qimage.setPixel(m_pointsArray[0].point.x(), m_pointsArray[0].point.y()+2,
                                m_pointsArray[0].maskColorIndex);
        }
                              ...
                              JointID id)
{
    ...
    static unsigned int id2color[SHADOW_JOINTS_NUM] =
    {
        ...
        m_torsoColorIndex,          // JointID_torso,
    ...
    if((x < m_width-1) && (y < m_height-1) && id == JointID_torso)
    {
        // get only torso joint
